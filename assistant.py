...
file = openai_client.files.create(
  file=open('./docs/presentation.txt','rb'),
  purpose='assistants'
)
...
assistant = openai_client.beta.assistants.create(
  ...
  tools=[{"type": "retrieval"}], # since we are adding files, we need to specify that we need to activate one tool which need files as `retrieval` or `code_interpreter`
  file_ids=[file.id] # files specified here will be available in to all threads of this assistant
)

# now to use the assistant we'll create a thread
thread = openai_client.beta.threads.create() # you can also add message when creating a thread
